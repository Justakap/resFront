...
export default function Home() {
    ...
    const downloadPDF = () => {
        ...
        const cgpa =
  data.semester > 4
    ? `${Math.min((totalCreditPoints / totalCredits).toFixed(2), 10.00)} - ${Math.min(((totalCreditPoints + 2) / totalCredits).toFixed(2), 10.00)}`
    : `${Math.min((totalCreditPoints / totalCredits).toFixed(2), 10.00)} - ${Math.min(((totalCreditPoints + 1) / totalCredits).toFixed(2), 10.00)}`;
    ...
    return (
        <div className="report-container">
            ...
            {/* <h3 className="clg-head m-3 no-underline">{data && data.College_name}</h3> */}
            <div className="input-file w-4/5">
              {url ? <></>:<> <p className=" mr-1 font-semibold text-sm text-nowrap">
                    Upload Result :
                </p></>}
